...
bool RCSwitch::NexusAvailableCode;

OregonDecoderV2 orscV2;
OregonDecoderV3 orscV3;
RCSwitch_ rcswp1;
PacketStorage storage;
Decoder decoder(storage, 5, 100, 100);
//CrestaDecoder cres;
//KakuDecoder kaku;
//XrfDecoder xrf;
//HezDecoder hez;
...
RCSwitch::RCSwitch(int rxpin, int txpin)
{
  ...
  RCSwitch::NexusAvailableCode = false;
...
// Set to true when a code has been decode by the
// Nexus modul
bool RCSwitch::NexusAvailable()
{
  return RCSwitch::NexusAvailableCode;
}
...
void RCSwitch::getNexusCode(char *_dest)
{
  ...
  {
    printf("Nowe odyczty %d \n", NewReadings.size());
    std::set<Packet>::iterator it = NewReadings.begin();
    while (it != NewReadings.end())
    {
      printf("%f, %d \n", it->GetTemperature(), it->GetHumidity());
      int n;
      double val2 = it->GetHumidity(), val1 = it->GetTemperature();
      n = val1 * 1000;
      memcpy(_dest, &n, 4);
...
void RCSwitch::handleInterrupt()
{
  ...
  { // avoid reentrance -- wait until data is read
    ...
    if (decoder.Decode(p))
    {
      RCSwitch::NexusAvailableCode = true;
    }
