...
class floatCharacteristic : public characteristic {
 ...
 public:
  floatCharacteristic(int aid, int iid, charType type, int permission,
                      float minVal, float maxVal, float step, unitType unit)
      : characteristic(aid, iid, type, permission, unit),
        minValue(minVal),
        maxValue(maxVal),
        step(step),
        unit(unit) {
...
class uint8Characteristic : public characteristic {
 ...
 public:
  uint8Characteristic(int aid, int iid, charType type, int permission,
                      int minVal, int maxVal, int step, unitType unit,
                      int value)
      : characteristic(aid, iid, type, permission, unit),
        minValue(minVal),
        maxValue(maxVal),
  ...
  virtual void setValue(std::string str) {
    ...
    if (value != newValue) {
      setValue(newValue);
      if (set_value_callback) set_value_callback(this, &this->aid);
    }
  }
};
...
class stringCharacteristic : public characteristic {
 ...
 public:
  stringCharacteristic(int aid, int iid, charType type, int permission,
                       std::string value,
                       unsigned short maxLen = HOMEKIT_STRING_DEFAULT_LENGTH)
      : characteristic(aid, iid, type, permission, unit_none),
        maxLength(maxLen),
        value(value) {
    setValue(value);
...
class boolCharacteristic : public characteristic {
 ...
 public:
  boolCharacteristic(int aid, int iid, charType type, int permission,
                     bool value)
