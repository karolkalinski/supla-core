...
void handleAccessory(const char *request, unsigned int requestLen, char **reply, unsigned int *replyLen, connectionInfo *sender) {
    ...
    if (strcmp(path, "/accessories") == 0) {
		...
		if (strncmp(method, "GET", 3) == 0) {
            ...
            supla_log(LOG_DEBUG, "get characteristics %s with len %d", indexBuffer, strlen(indexBuffer));
            ...
            while (strlen(buffer2) && statusCode != HTTP_STATUS_BAD_REQUEST) {
                ...
                if (accessory==NULL) {
                    ...
                    if (updateNotify) {
						...
						supla_log(LOG_DEBUG, "ask to notify one characteristics: %d . %d -> %s", aid, iid, value);
						...
						supla_log(LOG_DEBUG, "ask to change one characteristics: %d . %d -> %s", aid, iid, value);
