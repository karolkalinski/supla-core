...
class homekit_accessories {
 private:
  ...
  accessory* add_accessory_light_switch(
      accessory* accessory,
      set_value_callback_routine callback); /* lightswitch */
  accessory* add_accessory_dimmer(int accessoryId,
                                  service* info); /* lightswitch */
  accessory* add_accessory_rgb_lightning(int accessoryId,
                                         service* info); /* lightswitch */
  accessory* add_accessory_dimmer_and_rgb_lightning(
      int accessoryId, service* info); /* lightswitch */
  accessory* add_accessory_sensornonc(int accessoryId,
                                      service* info); /* occupancy sensor */
  accessory* add_accessory_thermostat(int accessoryId,
                                      service* info); /* thermostat */
  ...
  service* get_accessory_information_service(
      _supla_int64_t aid, std::string firmware_revision,
      std::string manufacturer, std::string model, std::string name,
