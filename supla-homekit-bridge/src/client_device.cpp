...
void set_value_callback_routine_execution(void* ptr, void* aid) {
  ...
  client_device_channel* channel =
      channels->find_channel(*((_supla_int64_t*)aid) - 1);
  ...
  if (channel) {
    switch (channel->getFunc()) {
      ...
      case SUPLA_CHANNELFNC_CONTROLLINGTHEGARAGEDOOR: {
        /* is not really mather that homekit sends open/close. in supla there is
         * an relay to run*/
        supla_client_open(sclient, channel->getId(), 0, 1);
      } break;
...
void client_device_channel::setHKValue(char value[SUPLA_CHANNELVALUE_SIZE],
  ...
  switch (this->getFunc()) {
    case SUPLA_CHANNELFNC_HUMIDITYANDTEMPERATURE: {
      service* service =
          accessory->getServiceByType(serviceType_temperatureSensor);
      ...
      floatCharacteristic* humidity =
          (floatCharacteristic*)service->getCharacteristicByType(
              charType_currentHumidity);

      active = (boolCharacteristic*)service->getCharacteristicByType(
          charType_sensorActive);
    ...
    case SUPLA_CHANNELFNC_POWERSWITCH: {
      ...
      boolCharacteristic* on =
          (boolCharacteristic*)service->getCharacteristicByType(charType_on);
    ...
    case SUPLA_CHANNELFNC_CONTROLLINGTHEROLLERSHUTTER: {
      ...
      boolCharacteristic* obstruction =
          (boolCharacteristic*)service->getCharacteristicByType(
              charType_obstruction);

      if (!obstruction) return;
      obstruction->setValue(!this->getOnline());

      uint8Characteristic* currentPosition =
          (uint8Characteristic*)service->getCharacteristicByType(
              charType_currentPosition);
    ...
    case SUPLA_CHANNELFNC_CONTROLLINGTHEGARAGEDOOR: {
      service* service =
          accessory->getServiceByType(serviceType_garageDoorOpener);
      ...
      uint8Characteristic* targetDoorState =
          (uint8Characteristic*)service->getCharacteristicByType(
              charType_targetDoorState);
      if (!targetDoorState) break;

      uint8Characteristic* currentDoorState =
          (uint8Characteristic*)service->getCharacteristicByType(
              charType_currentDoorState);
...
std::string client_device_channel::getName(void) { return this->name; }
std::string client_device_channel::getModel(void) { return this->model; }
...
bool client_device_channel::isHidden(void) { return !this->isHK; }
...
void client_device_channel::setFunc(int value) { this->Func = value; }
...
client_device_channels::client_device_channels() { this->initialized = false; }
...
client_device_channel* client_device_channels::add_channel(
  ...
  if (channel == 0) {
    channel = new client_device_channel(
        ...
        TextParam3, Hidden, Online, Caption, ++current_accessory_id);
    ...
    if (channel != NULL && safe_array_add(arr, channel) == -1) {
      ...
      if (!channel->isHidden())
        accessories->add_accessory_for_supla_channel(
            channel, identify_callback_routine_execution,
